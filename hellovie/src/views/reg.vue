...
export default {
  ...
  data() {
    return {
      users: {name: null, password: null, phone: null, mail: null}
    }
  },
  methods: {
    reging() {
      if (this.users.name != null && this.users.password != null) {
        this.users.name = this.users.name.replace(/\s*/g, "")
        this.users.password = this.users.password.replace(/\s*/g, "")
        if (this.users.name != "" && this.users.password != "") {
          console.log(this.users)
          let param = qs.stringify(this.users);
